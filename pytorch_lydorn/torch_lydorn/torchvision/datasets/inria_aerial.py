...
IMAGE_FILENAME_FORMAT = IMAGE_NAME_FORMAT + ".png"  # City name, number #MINA__CHANGE IT to TIF or PNG denepding on ur data
    ...
    def load_raw_data(self, tile_info):
        ...
        if self.gt_source == "disk":
            ...
            if self.gt_type == "npy":
                ...
                leng=len(geojson['features'])#['geometry'])
                data_list=[]
                for i in range(leng):
                    data_list.append(shapely.geometry.shape(geojson['features'][i]['geometry']))
                raw_data["gt_polygons"] =data_list# list(shapely.geometry.shape(geojson))'''
